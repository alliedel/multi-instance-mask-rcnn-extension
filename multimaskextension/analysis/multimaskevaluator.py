...
class MultiMaskCOCOEvaluator(COCOEvaluator):
    ...
    def get_imgEval_max_area_index(self, coco_eval):
        all_area = coco_eval.params.areaRng[0]
        assert all_area[0] == min(a[0] for a in coco_eval.params.areaRng)
        assert all_area[1] == max(a[1] for a in coco_eval.params.areaRng)

        image_id_lookup = np.array([imgId for catId in coco_eval.params.catIds
                                    for areaRng in coco_eval.params.areaRng
                                    for imgId in coco_eval.params.imgIds])
        cat_id_lookup = np.array([catId for catId in coco_eval.params.catIds
                                  for areaRng in coco_eval.params.areaRng
                                  for imgId in coco_eval.params.imgIds])
        area_rng_lookup = np.array([areaRng for catId in coco_eval.params.catIds
                                    for areaRng in coco_eval.params.areaRng
                                    for imgId in coco_eval.params.imgIds])
        assert len(image_id_lookup) == len(coco_eval.evalImgs)
        imgcat_indexer = {}
        for imgId in coco_eval.params.imgIds:
            imidxs = [i for i, x in enumerate(image_id_lookup) if x == imgId]
            for catId in coco_eval.params.catIds:
                imgcat_indexer[(imgId, catId)] = [i for i in imidxs if cat_id_lookup[i] == catId and
                                                  area_rng_lookup[i][0] == all_area[0] and area_rng_lookup[i][1] ==
                                                  all_area[1]][0]
        return imgcat_indexer

    def get_imgEval_indexer(self, coco_eval: COCOeval):
        image_id_lookup = np.array([imgId for catId in coco_eval.params.catIds
                                    for areaRng in coco_eval.params.areaRng
                                    for imgId in coco_eval.params.imgIds])
        cat_id_lookup = np.array([catId for catId in coco_eval.params.catIds
        ...
        for imgId in coco_eval.params.imgIds:
            imidxs = [i for i, x in enumerate(image_id_lookup) if x == imgId]
            for catId in coco_eval.params.catIds:
                imgcat_indexer[(imgId, catId)] = [i for i in imidxs if cat_id_lookup[i] == catId]

        return imgcat_indexer

    def filter_fcn(self, coco_eval: COCOeval):
        """
        # 'Clean' results of 'secondary' instance masks (that overlap with another mask
        # but come from a different proposal box)
        # We define these masks as:
        #   - iou > 0 with a GT that has already been matched to a different detection
        #   - the matched detection is not from the same proposal box
        #       ^-- this is the challenging one
        """

        # Conditions for keeping:
        # A [true positive]. If a mask is matched to a GT (and is therefore the best match to that GT, since we're
        # using aggregated predictions + matching)
        # OR
        # B [false positive]. If a mask has IOU=0 with all GT
        # OR
        # C [known duplicate]. If a mask in the same proposal box is unmatched but has IOU>0 with a GT matched to
        # another mask in its box.
        #
        # Complementary conditions (for discarding):
        # X [suboptimal positive]. If the only GT with which a mask has non-zero IOU are matched to masks outside of
        # its proposal box
        # - if a mask is matched to a GT, we do not discard it.
        # - else:
        # -- if a mask has iou overlap 0 with another GT, we discard it.
        imgcat_indexer = self.get_imgEval_indexer(coco_eval)
        imgcat_indexer_max_area = self.get_imgEval_max_area_index(coco_eval)
        for imgidx, image_id in enumerate(coco_eval.params.imgIds):
            for catidx, catid in enumerate(coco_eval.params.catIds):
                dt = coco_eval._dts[image_id, catid]
                # If no detections, don't need to filter dts
                if len(dt) == 0:
                    assert len(coco_eval.ious[(image_id, catid)]) == 0, 'Sanity check error'
                    continue
                # If no GT, all dts are false positives and we must keep them
                if len(coco_eval._gts[image_id, catid]) == 0:
                    assert len(coco_eval.ious[(image_id, catid)]) == 0, 'Sanity check error'
                    continue
                # This is how computeIOU sorts
                iou_dt_inds = np.argsort([-d['score'] for d in dt], kind='mergesort')
                dt_ids_for_iou = [dt[i]['id'] for i in iou_dt_inds]
                ious = coco_eval.ious[(image_id, catid)]
                # If no matches between gt and dt, continue
                if ious.sum() == 0:
                    continue
                all_area_eval_idx = imgcat_indexer_max_area[(image_id, catid)]
                res_all_areas = coco_eval.evalImgs[all_area_eval_idx]

                eval_idxs_same_catimg_by_area = imgcat_indexer[(image_id, catid)]
                maskious_per_gt_lst = []
                gtids = None
                eval_idxs = [x for x in eval_idxs_same_catimg_by_area if coco_eval.evalImgs[x] is not None]

                # Filter results from different instance size results
                for eval_idx in eval_idxs:
                    res = coco_eval.evalImgs[eval_idx]
                    print(res['dtIds'], res['gtIds'])
                    if res is None:
                        continue
                    assert res['image_id'] == image_id and res['category_id'] == catid, "Bug in indexing"
                    if gtids is None:
                        gtids = res['gtIds']
                    else:
                        assert all([x == y for x, y in zip(gtids, res['gtIds'])])
                    maskious_per_gt_lst.append(res['gtMatchIous'][0, :])
                if len(maskious_per_gt_lst) == 0:
                    continue
                maskious_per_gt = np.concatenate(maskious_per_gt_lst, axis=0)
                gt_matches = np.argmax(maskious_per_gt, axis=1)
                dt_matches = np.argmax(maskious_per_gt, axis=0)
                pass
...
def instances_to_json(instances, img_id):
    ...
    # def run_sanity_checks(self, coco_evals, predictions_per_mask_type_for_agg):
    #     log_and_print(self._logger, f"Running assertions")
    #     default_mask_i = 0
    #     default_mask = self.mask_names_agg[default_mask_i]
    #     for mask_name in self.mask_names_agg:
    #         for imid, (maskimeval, refimeval) \
    #                 in enumerate(zip(coco_evals[(mask_name, 'segm')].evalImgs,
    #                                  coco_evals[(default_mask, 'segm')].evalImgs)):
    #             # Assertions for sanity check:
    #             # 1. Image-category combos that have 'None' as an eval result should match up
    #             assert not xor(refimeval is None, maskimeval is None)
    #             if refimeval is None:
    #                 continue
    #
    #             # 2. Image, cat ids in list of evalImgs should match
    #             assert refimeval['image_id'] == maskimeval['image_id']
    #             assert refimeval['category_id'] == maskimeval['category_id']
    #
    #             # 3. For each image, the gtIds should match and be in the same order for all mask names
    #             assert all(x == y for x, y in zip(maskimeval['gtIds'], refimeval['gtIds']))
    #     # More assertions
    #     for mask_i, mask_name in enumerate(self.mask_names_agg):
    #         n_nones = 0
    #         assert len(predictions_per_mask_type_for_agg[mask_name]) == len(coco_evals[(mask_name, 'segm')].evalImgs)
    #             for imgpred, imgeval in zip(predictions_per_mask_type_for_agg[mask_name],
    #                                     coco_evals[(mask_name, 'segm')].evalImgs):
    #             if imgeval is None:
    #                 n_nones += 1
    #             else:
    #                 assert imgpred['image_id'] == imgeval['image_id']
    #         log_and_print(f"Mask {mask_name}: {n_nones} Nones")
