...
def get_font_scale(text, height_lim, width_lim, start_scale=1.0, decr=0.1, font=cv2.FONT_HERSHEY_SIMPLEX, line_type=2):
    assert 0 <= decr < 1.0
...
def get_offset_for_centered_text(text, box_shape_rc, font_scale, font=cv2.FONT_HERSHEY_SIMPLEX, line_type=2):
    text_width, text_height = cv2.getTextSize(text, font, font_scale, line_type)[0]
...
def write_text(img, text, lower_left_rc: (int, int), font=cv2.FONT_HERSHEY_SIMPLEX, font_scale=1,
               font_color=(255, 255, 255), line_type=2, enforce_fit_on_image=True, enforced_shape_rc=None,
               center_text=None):
    ...
    if enforce_fit_on_image:
        ...
        font_scale = get_font_scale(text, enforced_shape_rc[0], enforced_shape_rc[1], start_scale=font_scale, font=font)
        if center_text:
            offset_height, offset_width = get_offset_for_centered_text(text, enforced_shape_rc, font_scale)
    ...
    cv2.putText(img, text, (lower_left_rc[1], lower_left_rc[0]), font, font_scale, font_color, line_type)
    return font_scale


def collate_figures(figures, figure_name, exporter, delete_individuals=False, write_names_on_image=True,
    ...
    if write_names_on_image:
        # titles by default on top 1/10th of image.  We assume for now that all figures are the same size
        start_c = 0
        n_figs = len(figures)
        for imshape, f in zip(img_shapes, figures):
            ...
            font_scale = write_text(out_fig, text, lower_left_rc=(txt_height, start_c),
                                    enforced_shape_rc=(txt_height, txt_width), font_color=font_color, center_text=True,
                                    **textkwargs)
            # dbprint(font_scale)
            start_c += imshape[1]
...
def visualize_instancewise_groundtruth(datapoint, cfg, exporter, tag=None):
    ...
    colors = [
        vis._jitter([x / 255 for x in metadata.thing_colors[c]]) for c in datapoint['instances'].gt_classes
    ]
    alpha = 0.8
